...
const videoLinks = {
    ...
    popularQuestions: {
        ...
        '7': 'https://youtu.be/jkuTdRfkusM',
        '8': 'https://youtu.be/s2T0HrR1JIk',
        '9': 'https://youtu.be/qjCZKNWQ3u8',
...
async function handleRegionSelection(chatId, message) {
    ...
    if (region) {
        if (message === '7') {  // Проверяем, выбрана ли Актюбинская область (ключ '7')
            // Ответ только для Актюбинской области (только 1-й способ входа — по ЭЦП)
            await client.sendMessage(chatId, 
                `📍 ${region.name}\n` +
                `🌐 ${region.link}\n\n` +
                `🔑 Способ входа 👇\n\n` +
            ...
            // Ответ для всех остальных областей (оба способа входа)
            await client.sendMessage(chatId, 
                `📍 ${region.name}\n` +
                `🌐 ${region.link}\n\n` +
                `🔑 Способы входа 👇\n\n` +
...
function sendPopularQuestionsMenu(chatId) {
    client.sendMessage(chatId, 
        ...
        '7️⃣ ● Как в досье поменять год? (Штатное расписание)\n' +
        '8️⃣ ● Как в досье поменять год? (Штатное расписание)\n' +
        '9️⃣ ● Как в досье поменять год? (Штатное расписание)\n' +
