...
const regionLinks = {
    ...
    '8': { name: '🕌 Туркестанская область', link: 'https://turkistan.csi.kz' }
};
...
const userErrorCount = {}; // Храним ошибки по каждому пользователю
...
client.on('message', async msg => {
    ...
    // 🚫 Игнорируем медиафайлы (видео, изображения, документы, аудио)
    if (msg.type === 'video' || msg.type === 'image' || msg.type === 'document' || msg.type === 'audio') {
        console.log(`[INFO] Получен файл (${msg.type}) от ${chatId} — игнорирую.`);
        client.sendMessage(chatId, 'Извините, я не умею обрабатывать медиафайлы. Попробуйте отправить текстовое сообщение.');
        return;
    }
    ...
    case 'MAIN_MENU':
        ...
        if (/^[1-8]$/.test(message)) {  
            ...
            // Обнуляем счетчик ошибок при правильной команде
            userErrorCount[chatId] = 0;
            
            await handleMainMenuSelection(chatId, message);
        } else {
            // Если команды нет в списке, увеличиваем счетчик ошибок
            if (!userErrorCount[chatId]) {
                userErrorCount[chatId] = 0;  // Инициализация, если пользователя не было в списке
            }
            userErrorCount[chatId]++; // Увеличиваем счетчик

            console.log(`[WARN] Пользователь ${chatId} ввел некорректную цифру (${message}), попытка ${userErrorCount[chatId]}`);

            // Если ошибок меньше 3 — бот молчит
            if (userErrorCount[chatId] < 3) {
                return;
            }

            // После 3 ошибок отправляем предупреждение
            ...
            // Сбрасываем счетчик после отправки сообщения
            userErrorCount[chatId] = 0;
        }
        break;    
